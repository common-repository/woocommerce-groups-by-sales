...
class WooGroupsBySales {
	...
	public static function woocommerce_checkout_order_processed( $order_id ) {
		...
		if ( $levels > 0 ) {
			...
			switch ( $method ) {
				case "orders":
					$spent = self::woocommerce_get_num_orders( $user->data->user_email );
					break;
				case "total":
				default:
	...
	/**
	 * Gets the num. orders of the user with this email.
	 * @param  string $email Users email
	 * @return float
	 */
	public static function woocommerce_get_num_orders($email) {
